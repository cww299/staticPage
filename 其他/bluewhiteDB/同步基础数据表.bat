...
set "sourceHost=192.168.1.74"
set "targetHost=192.168.1.199"

set "Ymd=%date:~0,4%%date:~5,2%%date:~8,2%"
cd C:\Program Files\MySQL\MySQL Server 5.7\bin\
mysqldump -h%sourceHost% -uroot -p1234 bluewhite  %tableName% >C:\Users\Administrator\Desktop\bluewhiteDB\backup\bluewhie74_%Ymd%.sql
if errorlevel 1 (
    color 4
    echo basedata backup fail
) else (
    echo basedata backup success,start restore
    mysql -h%targetHost% -uroot -p1234 bluewhite < C:\Users\Administrator\Desktop\bluewhiteDB\backup\bluewhie74_%Ymd%.sql
    if errorlevel 1 (
        color 4
